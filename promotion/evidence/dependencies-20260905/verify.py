"""Verify the retained packet's raw bytes; optionally compare its effective dependency source binding."""
import argparse, hashlib, json, stat
from pathlib import Path

parser = argparse.ArgumentParser(description=__doc__)
parser.add_argument('--source-root', type=Path, help='Optional checkout with the exact reviewed 177 source files')
...
source_count = None
if args.source_root:
    bindings = json.loads((root / 'effective-source-binding.json').read_text(encoding='utf-8'))
